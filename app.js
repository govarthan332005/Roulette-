...
function setSlotValue(i, val){
  ...
  // Keep "Add Next" button state in sync whenever sequence changes
  if(typeof updateAddNextBtnState === 'function') updateAddNextBtnState();
}
...
document.getElementById('fillRecent').addEventListener('click', ()=>{
  ...
  updateAddNextBtnState();
  toast('Filled with recent history');
});

/* ============================================================
   SLIDING WINDOW — Add Next Number
   Slides existing values left: oldest (R-10) drops, new num
   becomes R-1 (newest). No need to re-enter all 10 numbers.
   ============================================================ */
function isSeqFull(){
  return seqValues.every(v => v !== '' && v !== null && v !== undefined);
}

function updateAddNextBtnState(){
  const btn = document.getElementById('addNextBtn');
  if(!btn) return;
  const full = isSeqFull();
  btn.disabled = !full;
  btn.querySelector('span').textContent = full
    ? '➕ ADD NEXT NUMBER (Slide)'
    : '➕ ADD NEXT — Fill all 10 slots first';
}

function slideInNumber(n, autoPredict){
  // Visual: animate old cells sliding out, then re-render with new values
  seqCells.forEach(c => {
    c.classList.remove('slide-in','slide-out');
    // Restart animation
    void c.offsetWidth;
  ...
  setTimeout(()=>{
    // Shift left: drop seqValues[0], push new number at end
    const newArr = [...seqValues.slice(1), String(n)];
    for(let i=0;i<10;i++){
      setSlotValue(i, newArr[i]);
    }
    seqCells.forEach(c=>{
    ...
    setActiveSlot(9);          // newest slot active
    updateAddNextBtnState();
    SFX.success(); haptic(12);
    toast('🔁 Slid in '+n+' — oldest number dropped');

    if(autoPredict){
      // Trigger predict after brief delay so slide animation completes
...
document.getElementById('addNextBtn').addEventListener('click', ()=>{
  if(!isSeqFull()){
    SFX.error(); haptic(15);
    return toast('Fill all 10 slots first, then use Add Next');
  }
  SFX.tap();
  keypadModal('Add Next Number', 'Tap the newest result — it will slide in as R-1 and the oldest will drop:', val=>{
    const n = parseInt(val,10);
    if(isNaN(n)||n<0||n>36){ SFX.error(); return toast('Invalid number'); }
    slideInNumber(n, true);  // auto-predict after slide
  });
});
...
document.getElementById('saveActualBtn').addEventListener('click', ()=>{
  ...
  keypadModal('Record Actual Result','Tap the actual winning number:', val=>{
    ...
    updateModelStatus();
    toast(n === lastPrediction.predicted ? '🎉 Exact hit! Model updated + slid in' : 'Result saved, model retrained + slid in');
    lastPrediction = null;
    renderHistory();

    // 🔁 Auto-slide: if sequence was full, slide this actual number into the window
    // so the user is INSTANTLY ready for the next prediction — no re-entry needed.
    if(isSeqFull()){
      slideInNumber(n, false);
    } else {
      // If slots not yet full, just append it at the first empty slot
      const firstEmpty = seqValues.findIndex(v => v === '' || v === null || v === undefined);
      if(firstEmpty !== -1){
        setSlotValue(firstEmpty, String(n));
        setActiveSlot(Math.min(9, firstEmpty + 1));
      }
    }
  });
});
...
renderHistory();
updateTrainDisplay();
updateAddNextBtnState();   // initialise the Add-Next button state
